...
/// a soft warm light near the top-left, a faint
...
/// Three soft pools of colour over the paper, fading out towards the bottom.
[[ stitchable ]] half4 aurora(
    float2 position, half4 color,
    float2 size, half4 a, half4 b, half4 c, float strength
) {
    float2 uv = position / max(size, float2(1.0));
    float aspect = size.y / max(size.x, 1.0);
    float2 p = uv * float2(1.0, aspect);

    float2 ca = float2(0.22, 0.08 * aspect + 0.06);
    float2 cb = float2(0.88, 0.12 * aspect);
    float2 cc = float2(0.50, 0.45 * aspect + 0.05);
