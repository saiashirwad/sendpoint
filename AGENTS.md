...
**Sources/Sendpoint/Settings.** The settings window (`SettingsWindowController`, `SettingsView`, and the panes), first-run setup (`SetupWindowController`, `SetupView`, `SetupTour`), and templates (`TemplateSettings`, `TemplateEditorController`, `SettingsTemplatesPane`). The template-editor machine, `TemplateEditorState`, lives in SendpointDomain. `AppSettings` holds the general preferences.
